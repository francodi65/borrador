...
%Parámetros
DataLength = 80000;         % Cantidad de símbolos a transmitir
FreqSymbol = 1.2e3;          % Frecuencia de transmisión de símbolos 
NumZeros = 9;                % Cantidad de ceros entre símbolos para asegurar cero ISI(interferencia entre símbolos)
...
EsNodB = 0:9;               % Determina la relación en decibeles entre la energia del símbolo Es y la densidad espectral de potencia de ruido No
EsNo = 10.^(EsNodB/10);
...
RespFiltro= fftshift(abs(fft(SRRCFilter)));
figure
...
%--------------------------------------------------------------------------
%Tarea
deltaw=(2*pi)/(length(Noise)-1);
w1=-pi:deltaw:pi;
deltaw=(2*pi)/(length(conv(Noise,SRRCFilter))-1);
w2=-pi:deltaw:pi;
figure
plot(w1,abs(fftshift(fft(Noise))));
figure
plot(w2,abs(fftshift(fft(conv(Noise,SRRCFilter)))));
figure
deltaw=(2*pi)/(length(xcorr(Noise))-1);
w1=-pi:deltaw:pi;
c= xcorr(Noise);
plot(w1,c);
figure
deltaw=(2*pi)/(length(xcorr(conv(Noise,SRRCFilter)))-1);
w2=-pi:deltaw:pi;
c= xcorr(conv(Noise,SRRCFilter));
plot(w2,c);
